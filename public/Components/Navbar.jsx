...
const Navbar = ({ setSearchDetails, setCurrentPage, setModal, bookmark, displayBookmark, bookmarkRef }) => {
	...
	return (
		<div className='header relative'>
			...
			<form onSubmit={(e) => handleClick(e)} className='flex search justify-between box-border pl-4 md:pl-10 absolute bottom-0 left-0 right-0   md:static'>
				<input type='text' placeholder='over 1Mil recipes eg mangoes, avocadoes' className='placeholder:text-[12px] h-[11px] text-[14px] w-1/12 md:w-full md:h-[23px] md:text-[1.7rem] flex-1 focus:outline-none ' onChange={(e) => setInputText(e.target.value)} />
			...
			<nav className=' flex md:justify-center md:items-center relative w-8 md:w-[332px] md:h-[100px]'>
				{isMenu ? (
					<span className='md:invisible visible text-[4xl] text-[#f48982]' onClick={(e) => closeMenu(e)}>
						<AiOutlineClose style={{ fontSize: '3rem' }} />
					</span>
				) : (
					<span className='md:invisible visible text-[4xl] text-[#f48982] ' onClick={(e) => openMenu(e)}>
				...
				<ul className='navigation menu__items'>
					...
					<li onMouseLeave={() => closeBookmark()} className='flex cursor-pointer w-[145px] justify-center gap-4 items-center text-[1.5rem]  font-semibold z-[10000] '>
						...
						<ul className='bookmark' ref={bookmarkRef}>
							{!bookmark.length && <p className='h-[200px] flex text-center justify-center items-center'>No bookmark yet. Find a nice recipe and bookmark it.</p>}
							{bookmark.map((item) => (
								<li key={item?.id} onClick={() => displayBookmark(item.id)}>
									<a className='flex  gap-8 py-[1.5rem] px-[3.25rem] items-center hover:bg-[#f2efee] cursor-pointer'>
										...
										<div>
											<h3 className='text-[#f38e82] max-w-[25rem] text-[1.45rem] overflow-hidden text-ellipsis whitespace-nowrap uppercase'>{item?.title}</h3>
